...
class GSAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, model, config, scheduler, **kwargs):
        adaptive = config.training.gsam.adaptive
        defaults = dict(adaptive=adaptive, **kwargs)
        super(GSAM, self).__init__(params, defaults)
        self.model = model
        self.base_optimizer = base_optimizer
        self.param_groups = self.base_optimizer.param_groups
        self.adaptive = config.training.gsam.adaptive
        self.perturb_eps = config.training.gsam.perturb_eps
        self.alpha = config.training.gsam.alpha
        self.max_grad_norm = config.training.gsam.max_grad_norm

        rho_scheduler = ProportionScheduler(
            pytorch_lr_scheduler=scheduler,
            max_lr=config.training.lr,
            min_lr=0.0,
            max_value=config.training.gsam.rho_max_value,
            min_value=config.training.gsam.rho_min_value,
        )
        ...
        grad_reduce = config.training.gsam.grad_reduce
...
class ProportionScheduler:
    def __init__(self, pytorch_lr_scheduler, max_lr, min_lr, max_value, min_value):
        ...
        assert (max_lr > min_lr) or ((max_lr == min_lr) and (max_value == min_value)), (
            ...
            "if `max_lr==min_lr` hence `lr` is constant with step, "
            "please set 'max_value == min_value' so 'value' is constant with step."
        )
